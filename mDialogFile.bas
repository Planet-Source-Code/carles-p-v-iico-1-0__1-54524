...
Private m_eViewMode  As Long
Private m_sDlgFilter As String
Private m_sCurExt    As String

'//

Public Function GetFileName(ByVal hwndOwner As Long, Optional sPath As String, Optional sFilter As String, Optional nFltIndex As Long, Optional sTitle As String, Optional OpenMode As Boolean = True, Optional ViewMode As Long = [ViewList]) As String
  ...
  Dim sExt As String
    ...
    If (lRet) Then
        ...
        If (OFN.nFileExtension = 0) And Len(m_sCurExt) > 2 Then
...
Private Function pvDialogHookProcess(ByVal hDlg As Long, ByVal wMsg As Long, ByVal wParam As Long, ByVal lParam As Long) As Long
...
Private Function pvGetFilterIndex(ByVal sPath As String) As Long
  ...
  Dim nStart As Long
    ...
    nStart = 1
    Do While nStart
        nStart = InStr(nStart + 1, m_sDlgFilter, Chr$(0), vbTextCompare)
        If (Mid$(m_sDlgFilter, nStart + 1, Len(sExt)) = sExt) Then Exit Do
        nIdx = nIdx + 1
    Loop
    pvGetFilterIndex = nIdx \ 2 + 1
End Function
